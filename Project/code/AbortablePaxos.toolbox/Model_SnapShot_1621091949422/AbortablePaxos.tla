...
(*--algorithm Paxos
 { variable AccMsg={}, PMsg={}; majority = (N-M+1) \div 2 + 1\* AccMsg: all Acceptor's receiving channel and PMsg: All Proposers' recv channel 
...
*)
\* BEGIN TRANSLATION (chksum(pcal) = "5471883c" /\ chksum(tla) = "bf931af1")
VARIABLES AccMsg, PMsg, majority, pc
...
vars == << AccMsg, PMsg, majority, pc, maxBal, acceptedValues, decided, b, s, 
           elected, lv, pVal >>
...
A(self) == /\ pc[self] = "A"
           ...
           /\ pc' = [pc EXCEPT ![self] = "A"]
           /\ UNCHANGED << AccMsg, majority, b, s, elected, lv, pVal >>
...
L(self) == /\ pc[self] = "L"
           ...
           /\ UNCHANGED << AccMsg, PMsg, majority, maxBal, acceptedValues, 
...
P1L(self) == /\ pc[self] = "P1L"
             /\ IF elected[self] # TRUE
                   ...
                   ELSE /\ pc' = [pc EXCEPT ![self] = "P2L"]
                        /\ UNCHANGED << AccMsg, b >>
             /\ UNCHANGED << PMsg, majority, maxBal, acceptedValues, decided, 
                             s, elected, lv, pVal >>
...
Validity == \A i \in AccMsg: i.type = "decide"
            => \E msg \in AccMsg: msg.type = "accept" /\ i.slot = msg.slot /\ i.dcd = msg.val[3] 
\* UC2 (Uniform Agreement) No two processes decide different values
Uniform_Agreement == \A pid1, pid2 \in Acceptor: \A i \in Slots: 
                     (decided[pid1][i] # {} /\ decided[pid2][i] # {}) 
                     => decided[pid1][i] = decided[pid2][i]
\* UC3 (Integrity) Each process can decide a value at most once
Integrity == \A pid1, pid2 \in Acceptor: 
...
\* Last modified Sat May 15 17:18:50 CEST 2021 by wangweiran
